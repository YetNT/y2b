...
const newCooldown = async (time, interaction, name, complex = false) => {
    ...
    if (complex === false || complex === undefined) {
      ...
      if (cooldown && cooldown[name][complex] && cooldown[name][complex].getTime() > date) {
        return;
      }
    }
    ...
    if (cooldown) {
      complex === false || complex === undefined ? cooldown[name] = new Date(date + cooldownTime) : cooldown[name][complex] = new Date(date + cooldownTime);
      await cooldown.save();
    } else {
      if (complex === false || complex === undefined ) {
 ...
 * @returns 
 */
const checkCooldown = async (name, interaction, EmbedBuilder,complex = false,custom = null) => {
    let query = {
        userId: interaction.user.id
    }
    let date = Date.now()
    ...
    let cooldown = await Cooldown.findOne(query)
    let result = (complex === false || complex === undefined) ? cooldown[name] : cooldown[name][complex]
